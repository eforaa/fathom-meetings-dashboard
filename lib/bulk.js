...
export function patchOf(set) {
    ...
    //архив: пометка «эта встреча больше не обновится». Только да и нет —
    //строка «true» из чужого запроса не должна превращаться в истину молча
    if ('archived' in set) {
        patch.archived = set.archived === true;
    }
...
export function splitByNeed(rows, patch) {
    ...
    for (const row of rows) {
        ...
        if ('archived' in patch && Boolean(row.archived) !== patch.archived) needs = true;
