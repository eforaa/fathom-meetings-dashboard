...
//--- где написано, что вообще можно нажимать ---------------------------------
//Половина списка управляется с клавиатуры, и до сих пор об этом знал только
//тот, кто читал код: j, k, стрелки, Enter, /, x, Shift+X, a, Escape. Функция,
//о которой никто не знает, не существует.
//
//«?» — общее место для такого списка: почта, GitHub, Linear открывают
//подсказку именно им. Знак вопроса набирается с шифтом, поэтому событие
//приходит с shiftKey — проверять его отдельно не нужно, важен сам символ.
export const opensHelp = (key, typing = false) => key === '?' && !typing;
