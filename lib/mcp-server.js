...
import { MAX_TYPES, meetingTypes } from './format.js';
...
async function listMeetings(args, ownerEmail) {
  ...
  //two columns hold this: types[] is what the picker and set_meeting_types
  //write (several per meeting), meeting_type is the older single value left by
  //the analysis. the screen reads them through meetingTypes(), which prefers
  //types[] and falls back to meeting_type — so the filter has to say the same
  //thing. matching meeting_type alone made a meeting that plainly shows a type
  //on screen invisible here, and could never match a meeting with two types.
  if (args?.type) {
    //the value goes into an or() string, so it is only ever one of ours
    if (!MEETING_TYPES.includes(args.type)) return { meetings: [], total: 0 };

    query = query.or(
      `types.cs.["${args.type}"],and(types.is.null,meeting_type.eq.${args.type})`,
    );
  }
  ...
  //Пробелы в данных: «дай следующие неразобранные».
  //
  //Это главный запрос, ради которого коннектор и открывают: 1041 встреча без
  //конспекта, и перебирать их вручную по одной — не работа. Отбор идёт в
  //базе, а не после выборки, иначе «дай двадцать неразобранных» вернуло бы
  //двадцать первых попавшихся, из которых неразобранных две.
  //
  //Пустая строка и пустой массив считаются отсутствием значения наравне с
  //null: Fathom и старый разбор оставляли и то, и другое.
  const NEEDS = {
    summary: (q) => q
      .or('summary.is.null,summary.eq.')
      .or('custom_summary.is.null,custom_summary.eq.'),
    type: (q) => q.or('types.is.null,types.eq.{}').is('meeting_type', null),
    rating: (q) => q.or('importance.is.null,importance.eq.0'),
  };

  if (args?.needs) {
    const narrow = NEEDS[args.needs];
    //неизвестное слово — это опечатка вызывающего, и молча отдать ему весь
    //список хуже, чем сказать, что так нельзя
    if (!narrow) {
      throw new Error(`needs: ожидается ${Object.keys(NEEDS).join(' | ')}, пришло «${args.needs}»`);
    }
    query = narrow(query);
  }
...
async function getStats(args, ownerEmail) {
  const { data, error } = await db
    .from('meetings')
    .select('meeting_type, types, analysis_status, date, duration_minutes')
    .eq('owner_email', ownerEmail)
    .limit(5000);
  ...
  const rows = data ?? [];
  const byType = {};
  //analysis_status is a leftover of the old night pipeline, which is gone.
  //reporting it as "pending" read like a queue that would eventually run —
  //there is none, so it is named for what it actually is
  const analysis = { analyzed: 0, not_analyzed: 0, failed: 0 };
  let minutes = 0;
  let earliest = null;
  let latest = null;

  for (const row of rows) {
    //the same rule the screen uses: types[] first, meeting_type as fallback.
    //a meeting with two types counts under both — that is what it is
    const types = meetingTypes(row);
    if (types.length === 0) byType.untyped = (byType.untyped ?? 0) + 1;
    for (const type of types) byType[type] = (byType[type] ?? 0) + 1;

    if (row.analysis_status === 'done') analysis.analyzed += 1;
    else if (row.analysis_status === 'failed') analysis.failed += 1;
    else analysis.not_analyzed += 1;
  ...
  return {
    total: rows.length,
    by_type: byType,
    //a meeting carrying two types is counted under each, so these can add up
    //to more than `total`
    analysis,
...
const TOOLS = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        needs: {
          type: 'string',
          enum: ['summary', 'type', 'rating'],
          description:
            'Only meetings missing something: summary (no конспект at all), type, or rating. This is how you get the next batch to work through — ask for needs:"summary" with a limit, fill them in, ask again.',
        },
...
//Object.create(null) — не украшение.
//
//Обычный объектный литерал наследует от Object.prototype, и поиск по имени
//находит там constructor, toString, valueOf, hasOwnProperty, __proto__. Проверка
//«нет такого инструмента» их пропускала: HANDLERS['constructor'] — функция, и
//вызов уходил дальше, возвращая клиенту сфабрикованный успех несуществующего
//инструмента. До базы это не доходило, но контракт «unknown tool» был сломан, а
//присланное имя возвращалось как удачный ответ.
//
//Пустой прототип убирает саму возможность: наследовать здесь нечего. Ниже, в
//разборе tools/call, стоит вторая проверка — что найденное действительно
//функция.
...
export async function handleMcpRequest(message, ownerEmail) {
  ...
  switch (method) {
    ...
    case 'tools/call': {
      //две проверки, а не одна: имя должно быть своим (не унаследованным) и
      //найденное должно быть функцией
      const name = params?.name;
      const handler = typeof name === 'string' && Object.hasOwn(HANDLERS, name)
        ? HANDLERS[name]
        : undefined;
