...
const TOOLS = [
  ...
  },
  {
    ...
    description:
      "The account's custom columns: their id, name and type (text, number, select, checkbox). Call this first to learn which fields you may fill with set_meeting_field.",
    inputSchema: { type: 'object', properties: {} },
  },
  {
    name: 'set_meeting_field',
    description:
      'Set a custom-column value on a meeting. Get the column_id from list_custom_columns. An empty value clears the cell. For a select column the value must be one the column offers.',
    inputSchema: {
      type: 'object',
      properties: {
        id: { type: 'string', description: 'Meeting id' },
        ...
        value: {
          ...
          description: 'The value to store, matching the column type',
        },
      },
  ...
  },
  {
    name: 'create_column',
    description:
      'Add a custom column to the table, the same one a person adds by hand. Types: text, number, select (a choice list), checkbox. For a select column pass options as the list of allowed values.',
    inputSchema: {
      type: 'object',
      properties: {
        ...
        options: {
          type: 'array',
          items: { type: 'string' },
          description: 'Allowed values, only for a select column',
        },
      },
      required: ['name', 'type'],
    },
