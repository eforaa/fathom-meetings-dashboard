...
import { toTsQuery, isMissingSearchColumn, safeLike } from './search-query.js';
...
const LIST_COLUMNS = [
    ...
    'importance',
    'custom_fields',
    //Конспекты. Их не было в списке вовсе, и это тихо ломало сразу три места:
    //строка под названием всегда оставалась пустой, фильтр «без конспекта»
    //считал такими ВСЕ встречи (229 из 229 при трёх настоящих), и панели
    //просмотра нечего было бы показать. Вес пустячный: все конспекты аккаунта
    //весят 2 КБ — расшифровки, которые действительно тяжёлые, сюда не входят.
    'summary',
    'custom_summary',
].join(', ');

//Отметка «архив» появляется вместе с db/archive-orphans.sql, а до того колонки
//в базе нет. Спрашивать несуществующую колонку — это ошибка на весь запрос, то
//есть пустая главная страница; поэтому она спрашивается отдельной попыткой, и
//при отказе список читается по-старому. Ровно так же устроен поисковый индекс.
const LIST_COLUMNS_WITH_ARCHIVE = `${LIST_COLUMNS}, archived`;

//postgrest отвечает на неизвестную колонку по-разному в зависимости от версии
...
export async function getMeetings({
    ...
    const build = (chunk, columns = LIST_COLUMNS_WITH_ARCHIVE) => {
        let query = db
            .from('meetings')
            .select(columns)
            .eq('owner_email', ownerEmail);

        //filtering by meeting type. types[] (what the picker writes, several
        //per meeting) wins, meeting_type is the older single column — the same
        //rule meetingTypes() applies on screen, so the filter cannot disagree
        //with what the row displays
        if (types.length) {
            const safe = types.filter((type) => MEETING_TYPES.includes(type));
            if (!safe.length) return EMPTY_RESULT;

            query = query.or(
                safe
                    .map(
                        (type) =>
                            `types.cs.["${type}"],and(types.is.null,meeting_type.eq.${type})`,
                    )
                    .join(','),
            );
        }
    ...
    try {
        meetings = ids
            ...
            ? sortRows(await fetchByIds(ids, (chunk) => build(chunk)), sortColumn, ascending)
            : await fetchAllPages(() => build(null));
    } catch (caught) {
        if (!missingColumn(caught.message)) throw caught;

        //db/archive-orphans.sql ещё не применён — читаем без отметки. Список
        //работает, просто архив от живых встреч пока не отличается
        console.warn('meetings: db/archive-orphans.sql is not applied, reading without the flag');
        meetings = ids
            ? sortRows(await fetchByIds(ids, (chunk) => build(chunk, LIST_COLUMNS)), sortColumn, ascending)
            : await fetchAllPages(() => build(null, LIST_COLUMNS));
    }
...
//The fast path: one index lookup instead of reading every transcript.
//
//Returns null — not an empty list — when the index is not there, which is how
//the caller knows to fall back. A meeting that genuinely matches nothing
//returns an empty list, and those two must not be confused.
async function searchByIndex(ownerEmail, term) {
    ...
    try {
        return await fetchAllPages(() =>
            db
                ...
                .select('id')
                .eq('owner_email', ownerEmail)
                .textSearch('search_doc', query, { config: 'simple' })
                .order('id', { ascending: true }),
        );
    } catch (caught) {
        //the column arrives with db/search-index.sql. until it is applied the
        //old path answers, so search keeps working — slowly, but correctly
        if (isMissingSearchColumn(caught.message)) {
            console.warn('search: db/search-index.sql is not applied, scanning instead');
            return null;
        }
        throw caught;
    }
...
export async function searchMeetingIds(ownerEmail, rawTerm) {
    ...
    //для запасного пути текст склеивается в строку фильтра PostgREST, поэтому
    //в него уходит только разрешённое — см. safeLike. Индексный путь этого не
    //требует: там значение передаётся отдельно от запроса
    const like = `%${safeLike(term)}%`;
    const ids = new Set();

    const indexed = await searchByIndex(ownerEmail, term);
    if (indexed) for (const row of indexed) ids.add(row.id);

    //text columns on the meeting itself (transcript included). only when the
    //index could not answer — reading every transcript is what this replaces
    const hits = indexed ? [] : await fetchAllPages(() =>
        db
            .from('meetings')
            .select('id')
            .eq('owner_email', ownerEmail)
